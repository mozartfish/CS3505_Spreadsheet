...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        ///   A recursive method used to determine which cells are visited given a starting cell 
        ///   and its dependencies. 
        /// </summary>
        private void Visit(String start, String name, ISet<String> visited, LinkedList<String> changed)
        {
            // Keep track of each cell we visit by adding it to the set
            visited.Add(name);
            foreach (String n in GetDirectDependents(name))
            {
                // If the path ends up where we started we have circular dependency
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                // If the cell we're at has not been visited, visit it.
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            changed.AddFirst(name);  // Keep track of cells that will need to be changed in GetCellsToRecalculate
        }
    }
}